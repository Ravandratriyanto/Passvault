...
I made Onyxlock to safely store passwords, PINs, API keys, and other important credentials, and I hope people find it useful. Right now it's only available for Windows, but versions for macOS, Linux, and Android are on the way.

## How it works (Da Baby Language)

Think of Onyxlock as a diary with a really smart lock. Everything you save goes into one file on your computer, but that file is scrambled beyond recognition unless you know your PIN.
...
There is no server. No cloud account. No telemetry. No "sync" that quietly uploads your vault somewhere. Onyxlock lives entirely on your disk. If you want a backup or to move to another device, Onyxlock generates a QR code (or a `.passbackup` file) and you decide when and how to move it.
...
```bash
git clone https://github.com/Ravandratriyanto/Onyxlock.git
cd Onyxlock
npm install
