...
<div class="screen">
  <div class="card">
    ...
    <div class="pad">
      ...
      {#if step === "pin"}
        <button class="key confirm" onclick={next} disabled={pin.length < MIN}>→</button>
      {:else}
        <button class="key confirm" onclick={submit} disabled={loading || confirmPin.length === 0}>
          {loading ? "…" : "✓"}
        </button>
      {/if}
    </div>
    ...
    {:else}
      ...
      <button class="text-btn" onclick={back} disabled={loading}>← Back</button>
    {/if}
  </div>
</div>
...
<style>
  ...
  .text-btn { background: transparent; color: #a0aec0; font-size: 13px; padding: 4px 8px; }
